...
// Ruta: /login      (porque está en app/(general)/login/page.tsx)
// Responsabilidad:
//   - Mostrar formulario de correo + contraseña
//   - Enviar esas credenciales a la API (loginApi)
//   - Guardar el token y el usuario en el store global (useAuthStore)
//   - Redirigir a /feed si el login es exitoso
//   - Si ya hay token (usuario logueado), redirigir automáticamente a /feed
...
export default function LoginPage() {
    ...
    //    así que lo redirigimos automáticamente al feed.
    useEffect(() => {
        if (token) {
            router.replace("/feed");
        }
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        try {
            ...
            // En el examen, esta función hará un fetch a la REST API del profe.
            const { token: newToken, user } = await loginApi(email, password);

            // Guardamos token y usuario en el store global y en localStorage.
            // Después, cualquier página podrá leer este token para hacer llamadas autenticadas.
            setAuth(newToken, user);

            // Redirigimos al feed una vez logueados correctamente
            router.push("/feed");
        } catch {
